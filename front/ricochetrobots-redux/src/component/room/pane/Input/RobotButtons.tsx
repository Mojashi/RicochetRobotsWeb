...
import { Dir } from "../../../../model/game/Dir"
...
export function RobotButtonsView({className, onTouch,handsLength, onRelease, selected} : Props) {
	...
	>
		...
		{Array.from({length:5}).map((_,idx)=>
			<RobotIconSvg key={idx} rid={idx} 
				onTouchStart={()=>{onTouch(idx) }}
				onTouchEnd={()=>{onRelease(idx)}}
				onTouchCancel={()=>{ onRelease(idx)}}
