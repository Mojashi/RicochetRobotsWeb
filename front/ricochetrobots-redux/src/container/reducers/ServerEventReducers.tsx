...
import { OptSubSample, Submission } from "../../model/game/Submission"
...
export const ServerEventReducers = {
    addSubmissionFromServer: (state:RoomState, action : PayloadAction<ServerSubmissionDto>) => (
        produce(state, draft => addSubmissionFunc(draft, action.payload.toSubmission(state.gameState?.participants)))
    ),
    ...
    setShortestFromServer: (state:RoomState, action : PayloadAction<ServerHiddenSubmissionDto>) => (
        produce(state, draft => setShortestFunc(draft, action.payload.toSubmission(state.gameState?.participants)))
    ),
    finishProblemFromServer : (state:RoomState,action : PayloadAction<ServerSubmissionDto[]>) => (
        produce(state,draft => finishProblemFunc(draft, action.payload.map(sub => sub.toSubmission(draft.gameState?.participants))))
    )
}