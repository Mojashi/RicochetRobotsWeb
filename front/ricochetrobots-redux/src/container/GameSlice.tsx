...
import {  Submission, ResultSubmission,  MySubmission } from "../model/game/Submission"
...
export const leaderBoardSelector = createSelector(selectRoomState, state=>{
	if(state.gameState)
		return Object.entries(state.gameState.points).map(
			([userID, point] ) => ({user : state.participants[parseInt(userID)] , point: point, online:state.onlineUsers[parseInt(userID)]})
		).sort((a,b)=>b.point - a.point)
	return Object.entries(state.onlineUsers).filter(([,online])=>online).map(
		([userID, online] ) => ({user : state.participants[parseInt(userID)] , point: 0, online:online})
	)
})
